...
public partial class BaseStateUpdateSystem<T> : SystemBase where T : struct, IComponentData, IMyState
{
    ...
    {
        base.OnCreate();
        _ecbSystem = World.GetOrCreateSystem<StateUpdateECBSystem>();
        _entityQuery = GetEntityQuery(typeof(T), typeof(MyStateMachine), typeof(Translation), typeof(Rotation), typeof(NonUniformScale));
    }
...
[BurstCompile]
public partial struct StateUpdateJob<T> : IJobEntityBatch where T : struct, IComponentData, IMyState
{
    ...
    public void Execute(ArchetypeChunk batchInChunk, int batchIndex)
    {
        ...
        for (int i = 0; i < batchInChunk.Count; i++)
        {
            ...
            state.OnStateUpdate(ref refData, in inData);

            // Handle Transitions
            if (refData.StateMachine.TransitionToStateIndex >= 0)
            {
                ECB.AddComponent(batchIndex, entity, new OnStateExit { NextState = (MyState.TypeId)refData.StateMachine.TransitionToStateIndex });
